...
class ReviewList(Resource):
    ...
    def post(self):
        ...
        # FIXED: Extract user ID from JWT object
        review_data["user_id"] = current_user["id"]  # Changed from current_user to current_user["id"]

        place = facade.get_place(review_data["place_id"])
        if not place:
            return {'error': 'Place not found'}, 404

        # FIXED: Compare owner_id with user ID string, not JWT object
        if place["owner_id"] == current_user["id"]:  # Changed from current_user to current_user["id"]
...
class ReviewResource(Resource):
    ...
    def put(self, review_id):
        ...
        try:
            ...
            # FIXED: Compare with user ID string, not JWT object
            if review.user_id != current_user["id"]:  # Changed from current_user to current_user["id"]
    ...
    def delete(self, review_id):
        ...
        current_user = get_jwt_identity()
        # FIXED: Extract user ID from JWT object
        current_user_id = current_user["id"]  # Changed from current_user to current_user["id"]
